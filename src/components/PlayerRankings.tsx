  ...
  return <section id="player-rankings" className="py-16 bg-secondary">
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {rankingCategories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${activeCategory === category ? "bg-foreground text-background" : "bg-card border border-border text-foreground hover:bg-muted"}`}>
              {category}
            </button>)}
        </div>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {currentPlayers.map(player => <tr key={player.rank} className="border-b border-border last:border-0 hover:bg-muted/50">
                    <td className="p-4">
                      <span className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold text-white ${player.rank === 1 ? "bg-primary" : player.rank === 2 ? "bg-gray-500" : player.rank === 3 ? "bg-orange-dark" : "bg-muted-foreground"}`}>
                        {player.rank}
                      </span>
                    </td>
        ...
        {/* Pagination */}
        <div className="mt-6 flex items-center justify-between gap-4 my-[4px]">
          ...
          <Pagination>
            <PaginationContent>
              ...
              {getVisiblePages().map((page, index) => <PaginationItem key={index}>
                  {page === "ellipsis" ? <PaginationEllipsis /> : <PaginationLink onClick={() => handlePageChange(page)} isActive={currentPage === page} className="cursor-pointer">
                      {page}
                    </PaginationLink>}
                </PaginationItem>)}
              
              <PaginationItem>
                <PaginationNext onClick={() => handlePageChange(currentPage + 1)} className={currentPage === TOTAL_PAGES ? "pointer-events-none opacity-50" : "cursor-pointer"} />
              </PaginationItem>
            </PaginationContent>
          </Pagination>
        </div>
